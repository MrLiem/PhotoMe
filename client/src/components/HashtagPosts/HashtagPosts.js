...
// Componet Tìm posts theo hashtag
const HashtagPosts = ({ token, showModal, showAlert }) => {
  ...
  // Lấy hashtag từ params
  const { hashtag } = useParams();
  const history = useHistory();

  // Show model khi click vào post
  ...
  const retrievePosts = async (offset) => {
    try {
      ...
      // fetching mỗi lần 20 posts rồi render lại
      response.posts
        ? setPosts((previous) => ({
  ...
  // Function render skeleton khi đang load posts
  ...
  // Callback function để fetching posts mỗi lần scroll
  ...
  // Ref tới function retrivePost để re-render mỗi lần fetching dc posts mới
  ...
  // TH ko tìm dc posts nào vs hashtag
  return !posts.fetching && posts.posts.length === 0 ? (
    ...
    <Fragment>
      ...
      {/* Show ImageGrid với hashtag tìm thấy */}
      <ImageGrid>
        {posts.posts &&
